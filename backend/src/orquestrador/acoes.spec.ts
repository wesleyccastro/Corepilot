...
describe('calcularAcoes', () => {
  it('etapa de aprovação sem loop tem só a ação Aprovar', () => {
    const acoes = calcularAcoes(
      { tipo: 'aprovacao', loopParaEtapaId: null },
      'etapa-2',
    );
  ...
  it('etapa de aprovação com loop ganha a ação Solicitar ajustes, exigindo motivo_correcao', () => {
    const acoes = calcularAcoes(
      { tipo: 'aprovacao', loopParaEtapaId: 'etapa-1' },
      'etapa-2',
    );
  ...
  it('etapa de interação do usuário tem só a ação Concluir', () => {
    const acoes = calcularAcoes(
      { tipo: 'interacao_usuario', loopParaEtapaId: null },
      'etapa-3',
    );
    expect(acoes).toEqual([
      {
  ...
  it('etapas automáticas/de agente/integração não têm ações (avançam sozinhas)', () => {
    expect(
      calcularAcoes(
        { tipo: 'tarefa_agente', loopParaEtapaId: null },
        'etapa-x',
      ),
    ).toEqual([]);
    expect(
      calcularAcoes(
        { tipo: 'decisao_automatica', loopParaEtapaId: null },
        'etapa-x',
      ),
    ).toEqual([]);
    expect(
      calcularAcoes({ tipo: 'integracao', loopParaEtapaId: null }, 'etapa-x'),
    ).toEqual([]);
  });
