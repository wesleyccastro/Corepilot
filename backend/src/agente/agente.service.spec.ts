...
describe('AgenteService', () => {
  ...
  it('propaga o NotFoundException se o módulo não for da empresa (não cria o agente)', async () => {
    ...
    await expect(
      service.create('modulo-x', 'empresa-1', {
        nome: 'X',
        funcao: 'Y',
        objetivo: 'Z',
      }),
  ...
  it('findByIdInEmpresa lança NotFoundException se o agente não existir ou não for da empresa', async () => {
    ...
    await expect(
      service.findByIdInEmpresa('agente-x', 'empresa-1'),
    ).rejects.toThrow(NotFoundException);
  ...
  it('update atualiza só os campos informados, escopado à empresa', async () => {
    ...
    (prisma.agente.update as jest.Mock).mockResolvedValue({
      id: 'agente-1',
      nome: 'Novo nome',
    });
    const service = new AgenteService(prisma, moduloService);

    const resultado = await service.update('agente-1', 'empresa-1', {
      nome: 'Novo nome',
    });

    expect(prisma.agente.findFirst).toHaveBeenCalledWith({
      where: { id: 'agente-1', empresaId: 'empresa-1' },
    });
