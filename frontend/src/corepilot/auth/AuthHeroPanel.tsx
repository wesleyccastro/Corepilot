...
export function AuthHeroPanel() {
  return (
    <div style={{ flex: 1, background: colors.navy, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: 60, position: 'relative', overflow: 'hidden' }}>
      ...
      <p style={{ position: 'relative', color: 'rgba(255,255,255,.8)', fontSize: 14, textAlign: 'center', maxWidth: 360, lineHeight: 1.6, margin: '0 0 28px' }}>
        Agentes de IA conectados aos dados e processos da sua empresa — do jeito que cada equipe precisa.
      </p>
      <img
        src={logoIcon}
        alt=""
        style={{ position: 'relative', height: 120, width: 'auto', filter: 'drop-shadow(0 12px 30px rgba(0,0,0,.3))' }}
      />
    </div>
  );
}
